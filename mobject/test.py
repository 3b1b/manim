...
def test_apply_over_attr_arrays():
    ...
    for attr in m.get_array_attrs():
        setattr(m, attr, np.array(
            [[1, 1, 0],
             [0, 1, 0],
             [0, 1, 1]]
        ))
    m.apply_over_attr_arrays(lambda x: x.T)
    for attr in m.get_array_attrs():
...
def test_update(monkeypatch):
    ...
    c = Circle()
    c.add_updater(mock_updater_1)
    c.add_updater(mock_updater_2)
    c.update(1)
    mock_updater_1.assert_called_once_with(c)
    mock_updater_2.assert_called_once_with(c, 1)
    c.add_updater(mock_updater_3)
    with pytest.raises(Exception):
        c.update(1)
