...
class Network(object):
    def __init__(self, sizes, non_linearity = "sigmoid"):
        ...
        if non_linearity == "sigmoid":
            self.non_linearity = lambda z: 1.0/(1.0+np.exp(-z))
            self.d_non_linearity = lambda z: \
                        (1.0/(1.0+np.exp(-z)))*(1-(1.0/(1.0+np.exp(-z))))

        #In 'old_projects/nn/network.py' 3b1b implemented a ReLU model, that is
        # frankly *way* outside the scope of this file. The only reason sigmoid
        # is implemented, 1) it fits into a lambda function, and 2) It means I 
        # dont have to rewrite this entire file. -MrPainting
...
class NetworkMobject(VGroup):
    ...
    def get_layer(self, size):
        ...
        #Instantiate a VGroup of 'neurons'(circles)
        neurons = VGroup(*[
            Circle(
        ...
        #Itteratively adds edge attributes to each neuron
        ...
        #This adds an ellipsis ("...") between neurons if there are more in the 
...
class TestNetwork(NetworkScene):
    def construct(self):
        ...
        self.wait()
        self.play(
            DrawBorderThenFill(network_mob.layers), 
            lag_ratio = 0.5,
            run_time = 2,
            rate_func=linear
        )
