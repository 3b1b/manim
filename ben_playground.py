...
class TestScene(ThreeDScene):
    ...
    def construct(self):
        ...
        self.play(
            ApplyMethod(self.camera.rotation_mobject.move_to, target_point)
        )

        screen = Square(fill_color = RED).rotate(TAU/4, axis = DOWN)
        self.add(screen)
        self.wait()

        source_point = [-3,0,0]
        light = LightSource().move_source_to(source_point)
        light.set_screen(screen)
        light.spotlight.camera = self.camera
        light.spotlight.update()
        self.add_foreground_mobject(light.spotlight.shadow)
        
        tracker = ScreenTracker(light.spotlight)
        self.add(tracker)

        self.play(
            SwitchOn(light.spotlight),
        )

        self.play(Rotate(screen,TAU/8))

        phi1 = 20 * DEGREES # angle from zenith (0 to 180)
        theta1 = -15 * DEGREES # azimuth (0 to 360)
        distance1 = 5
        target_point = self.camera.get_spherical_coords(phi1, theta1, distance1)

        print "spotlight points:"
        print light.spotlight.submobjects[4].get_anchors()[:10]

        print "shadow points:"
        print light.spotlight.shadow.get_anchors()
