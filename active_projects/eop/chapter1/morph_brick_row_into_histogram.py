...
class GenericMorphBrickRowIntoHistogram(Scene):
    ...
    def construct(self):
        ...
        self.bars = OutlineableBars(*[self.row.rects[i]
                                      for i in range(self.level + 1)])
        self.bar_anchors = [self.bar_anchor_height * UP + self.row.height *
                            (i - 0.5 * self.level) * RIGHT for i in range(self.level + 1)]
...
class MorphBrickRowIntoHistogram3(GenericMorphBrickRowIntoHistogram):
    ...
    def construct(self):
        ...
        for i in range(0, self.prob_denominator + 1):
            ...
            y_guide.move_to(self.bar_anchor_height * UP + i *
                            float(self.row.width) / self.prob_denominator * UP)
            y_guide_label = TexMobject(
                "{" + str(i) + "\over " + str(self.prob_denominator) + "}", color=GRAY)
...
class MorphBrickRowIntoHistogram20(GenericMorphBrickRowIntoHistogram):
    ...
    def construct(self):
        ...
        for (guide, i, h) in zip(y_guides, prob_grid, y_guide_heights):
            new_y_guide_height = self.bar_anchor_height + \
                i * self.scale_y * float(self.row.width)
