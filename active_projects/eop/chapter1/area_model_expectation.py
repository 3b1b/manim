...
class IllustrateAreaModelExpectation(Scene):
    ...
    def construct(self):
        ...
        formula = TexMobject(
            "E[X] = \sum_{i=1}^N p_i x_i").move_to(3 * LEFT + UP)
        ...
        for (p, string, bar) in zip(probabilities, prob_strings, hist.bars):
            ...
            p_label = TexMobject(string).next_to(
                brace, DOWN, buff=SMALL_BUFF).scale(0.7)
        ...
        one_p_label = TexMobject(str(1)).next_to(
            one_brace, DOWN, buff=SMALL_BUFF).scale(0.7)
