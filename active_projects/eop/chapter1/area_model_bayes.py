...
class IllustrateAreaModelBayes(Scene):
    ...
    def construct(self):
        ...
        label_not_A = TexMobject("P(\\text{not }A)").next_to(
            brace_not_A, DOWN).scale(0.7)
        ...
        label_not_B = TexMobject("P(\\text{not }B)").next_to(
            brace_not_B, LEFT).scale(0.7)
        ...
        label_B.target = TexMobject("P(B\mid A)").scale(
            0.7).next_to(brace_B.target, LEFT)
        ...
        self.play(
            label_B_knowing_A_copy.next_to, indep_formula.get_part_by_tex(
                "\cdot"), RIGHT,
        )

        # solve formula for P(B|A)

        rearranged_formula = TexMobject(
            "P(B\mid A)", "=", "{P(A\\text{ and }B) \over P(A)}")
        ...
        self.play(
            ...
            label_B_knowing_A_copy.move_to, rearranged_formula.get_part_by_tex(
                "P(B\mid A)"),
            label_A_copy.move_to, rearranged_formula[-1][10],
            label_A_and_B_copy.move_to, rearranged_formula[-1][3],
            indep_formula.get_part_by_tex(
                "=").move_to, rearranged_formula.get_part_by_tex("="),
            Transform(indep_formula.get_part_by_tex(
                "\cdot"), rearranged_formula[2][8]),
        )

        rect = SurroundingRectangle(
            rearranged_formula, buff=0.5 * MED_LARGE_BUFF)
