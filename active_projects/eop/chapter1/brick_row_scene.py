...
class BrickRowScene(PiCreatureScene):
    ...
    def construct(self):
        ...
        self.play(
            LaggedStartMap(
                Succession, coin_seqs, lambda m: (
                    FadeIn(m, run_time=0.1), MoveToTarget(m)),
                run_time=5,
                lag_ratio=0.5
            )
        )
        ...
        v = self.row.get_outcome_centers_for_level(
            2)[0] - single_flip_labels[0].get_center()
        self.play(
        ...
        v = self.row.get_outcome_centers_for_level(
            2)[-1] - single_flip_labels_copy[-1].get_center()
        self.play(
        ...
        for i in range(N):
            for j in range(N):
                ...
                dice_copy.shift(j * dice_width * RIGHT +
                                i * dice_height * DOWN)
        ...
        right_target_pos = 0.5 * (self.row.get_outcome_centers_for_level(3)
                                  [2] + self.row.get_outcome_centers_for_level(3)[3])
        ...
        probs = VGroup(
            *[TexMobject("{" + str(k) + "\over 8}") for k in counts])
        ...
        for i in range(n + 2):
            ...
            grouped_target_outcomes.add(
                VGroup(target_outcomes[index:index + size]))
            index += size
        ...
        for i in range(n + 1):
            self.play(
                ...
                Transform(grouped_outcomes_copy[i][0],
                          grouped_target_outcomes[i +
                                                  1][0][:old_tally_sizes[i]]
                          )
            )
        return
        ...
        self.play(
            Transform(grouped_outcomes_copy[k - 1],
                      original_grouped_outcomes[k - 1])
        )
        ...
        self.play(
            Transform(
                grouped_outcomes[k][0], grouped_target_outcomes[k][0][old_tally_sizes[k - 1]:]),
            Transform(
                grouped_outcomes_copy[k - 1][0], grouped_target_outcomes[k][0][:old_tally_sizes[k - 1]]),
        )
...
class ShowOutcomesInBrickRow4(BrickRowScene):
    ...
    def construct(self):
        ...
        heads_labels = VGroup(*[outcome.label[-1]
                                for outcome in new_outcomes_left])
        tails_labels = VGroup(*[outcome.label[-1]
        ...
        self.play(
            Transform(previous_outcomes_copy[0], new_outcomes_right[0], run_time=run_time))
        self.play(Transform(
        ...
        self.play(
            Transform(previous_outcomes_copy[7], new_outcomes_right[7], run_time=run_time))
