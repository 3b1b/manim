...
class UprightCoin(Circle):
    ...
    def __init__(self, **kwargs):
        ...
        self.symbol_mob = TextMobject(
            self.symbol, stroke_color=self.stroke_color)
...
class FlatCoin(UprightCoin):
    ...
    def __init__(self, **kwargs):
        ...
        control_points2 = self.copy().shift(
            self.thickness * DOWN).points[12:25].tolist()
        edge_anchors_and_handles = control_points1
        edge_anchors_and_handles.append(
            edge_anchors_and_handles[-1] + self.thickness * DOWN)
        edge_anchors_and_handles.append(
            edge_anchors_and_handles[-1] + self.thickness * UP)
        ...
        ridge_color = interpolate_color(
            self.stroke_color, self.fill_color, 0.7)
        for x in ridge_positions:
            y = -(1 - (x/self.radius)**2)**0.5 * \
                self.foreshortening * self.radius
            ridge = Line(
                x * RIGHT + y * UP,
