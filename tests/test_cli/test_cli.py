...
def test_help(python_version):
    ...
    assert (
        exitcode == 0
    ), f"Manim has been installed incorrectly. Please refer to the troubleshooting section on the wiki. Error:\n{err.decode()}"
...
def test_basicScene(python_version):
    ...
    assert os.path.exists(
        os.path.join(
            path_output, "videos", "basic_scenes", "480p15", "SquareToCircle.mp4"
        )
    ), err
    rmtree(path_output)
...
def test_WriteStuff(python_version):
    ...
    assert exitcode == 0, err
    assert os.path.exists(
        os.path.join(path_output, "videos", "basic_scenes", "480p15", "WriteStuff.mp4")
    ), err
    rmtree(path_output)
