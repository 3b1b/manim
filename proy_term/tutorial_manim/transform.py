...
class EjemploTransform(Scene):
	...
	def transform_lista(self):
		...
		esc1=1.6
		self.Ap=self.lista1[1][2].copy().scale(esc1).move_to(self.pantalla).set_color(WHITE)
		self.Bp=self.lista1[2][2].copy().scale(esc1).set_color(WHITE)
		self.Cp=self.lista1[3][2].copy().scale(esc1).set_color(WHITE)
		self.Dp=self.lista1[4][2].copy().scale(esc1).set_color(WHITE)
