...
class SingleStringTextMobject(SVGMobject):
    ...
    def __init__(self, text, **config):
        ...
        self.replace_space_char(file_name)
    ...
    def replace_space_char(self, file_name):
        file = open(file_name, "r")
        xmltext = file.read()
        file.close()
        xmltext = xmltext.replace("d=\"\"", "d=\"" + self.get_space_path_str() + "\"")
        file = open(file_name, "w")
        file.write(xmltext)
        file.close()

    def get_space_path_str(self):
        self.create_space_file()
        dir_name = consts.TEXT_DIR
        hash_name = "space"
        file_name = os.path.join(dir_name, hash_name) + '.svg'
        file = open(file_name, "r")
        xmlstr = file.read()
        temp = xmlstr.split("<path ")[1]
        i = temp.find("d=\"")
        j = temp.find("/>")
        pathstr = temp[i + 3:j - 1]
        temp = pathstr.split("L ")
        x1 = float(temp[1].split(" ")[0])
        x2 = float(temp[2].split(" ")[0])
        empty_area_path_str = "M " + str(x1) + " 0 L " + str(x2) + " 0 Z M " + str(x1) + " 0 "
        return empty_area_path_str

    def create_space_file(self):
        # anti-aliasing
        size = self.size * 10
        lsh = self.lsh * 10
