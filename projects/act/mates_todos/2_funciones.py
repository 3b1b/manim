...
class Receta(Scene):
	...
	def mostrar_receta(self):
		...
		procedimiento=VGroup(Texto("""
			...
			""",alignment="\\justify",color=BLACK).scale(0.5)).arrange_submobjects(DOWN,buff=0.1,aligned_edge=LEFT)
		...
		procedimiento_funcion=Formula("x","\\cdot","a","+","{","y","+","z","\\over","b","}",
			...
			"b":self.color_constantes},color=BLACK)
		...
		funcion=Formula("f","(","x",",","y",",","z",")=","x","\\cdot","a","+","{","y","+","z","\\over","b","}",
			...
			"f":self.color_titulo},color=BLACK).scale(0.9)
		...
		texto_brazo_variables=brazo_variables.get_text("Pueden variar").set_color(GRAY)

		brazo_constantes=Brace(constantes,RIGHT)
		texto_brazo_constantes=brazo_constantes.get_text("No varían").set_color(GRAY)
