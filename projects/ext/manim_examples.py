...
class MechanismScene(Scene):
	...
	def construct(self):
		...
		titulo=Texto("\\sc Mechanisms animations.",color=WHITE).scale(1.5).to_corner(UL).shift(RIGHT)
...
class CreatedBy(Scene):
	def construct(self):
		texto=Texto("\\sc This tool was created by:",color=WHITE).to_edge(UP)
		grant=ImageMobject("grant",height=5)
		i3b1b=ImageMobject("3b1b",height=5)
		imagenes=Mobject(grant,i3b1b).arrange_submobjects(RIGHT).next_to(texto,DOWN,buff=1)
...
class WhatCanDo(Scene):
	def construct(self):
		texto=VGroup(
			Texto("\\sc What animations",color=WHITE),
			Texto("\\sc can we do?",color=WHITE)
...
class MusicExample(MusicalScene):
    ...
    def agregar_escenario(self):
        ...
        titulo=Texto("\\sc In general, as far as your imagination can reach.",color=BLACK).to_corner(UL)
...
class Plot2D(GraphScene):
    ...
    def add_title(self):
        title = self.title = TextMobject("\\sc Animations of 2D graphs.")
        title.to_edge(UP)
...
class MathematicalProcess(Scene):
	def construct(self):
	...
	def imprime_formula(self):
		titulo=Texto("\\sc Animations of mathematical processes.",color=WHITE).to_corner(UL)
		ul=underline(titulo)
		formulas_tex=Texto("Formulas written on \\LaTeX",color=TEAL).to_edge(DOWN)
...
class PythagoreanProof(Scene):
	...
	def pre_cuadrado(self):
		...
		titulo=Texto("\\sc Animated mathematical proofs.",color=WHITE).to_corner(UL)
		ul=underline(titulo)
...
class Surfaces(ThreeDScene):
	def construct(self):
		...
		#sphere.shift(IN)
		question = TextMobject("3D Animations.",color=WHITE)
...
class MiniaturaInstallGNULinux(Scene):
    def construct(self):
        texto=Texto("Installation on\\\\","\\sc GNU/Linux")\
             .scale(4).set_color_by_gradient(BLUE,GREEN)
        texto[1].scale(1.2)
        VGroup(texto[0],texto[1]).move_to(ORIGIN)
        self.add(texto)

class MiniaturaInstalacionGNULinux(Scene):
    def construct(self):
        texto=Texto("Instalación en\\\\","\\sc GNU/Linux")\
             .scale(4).set_color_by_gradient(BLUE,GREEN)
        texto[1].scale(1.2)
        VGroup(texto[0],texto[1]).move_to(ORIGIN)
        self.add(texto)
