...
class Pendulum(ReconfigurableScene):
    ...
    def get_trajectory_dots(self, n_dots=40, color=YELLOW):
        ...
        def angle_to_point(a): return np.cos(a)*RIGHT + np.sin(a)*UP
        dots = VGroup(*[
...
class BoundedRadiusOfConvergence(CubicAndQuarticApproximations):
    ...
    def construct(self):
        self.setup_axes()
        def func(x): return (np.sin(x**2 + x)+0.5) * \
            (np.log(x+1.01)+1)*np.exp(-x)
        graph = self.get_graph(
            func, color=self.colors[0],
            x_min=-0.99,
            x_max=self.x_max,
...
class RadiusOfConvergenceForLnX(ExpGraphConvergence):
    ...
    def show_converging_point(self):
        ...
        self.play(
            approx_dot.move_to, self.coords_to_point(
                self.divergent_example, 0),
            *it.chain(
