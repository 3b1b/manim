...
class RefreshOnDerivativeDefinition(GraphScene):
    ...
    def construct(self):
        ...
        covered_new_deriv_parts = list(
            it.chain(*list(deriv_to_new_deriv.values())))
        uncovered_new_deriv_parts = [
            part for part in new_deriv if part not in covered_new_deriv_parts]
...
class LimitCounterExample(GraphLimitExpression):
    ...
    def approach_zero(self):
        ...
        def update_lines(lines, alpha):
            ...
            x_val = interpolate(sign*self.big_delta,
                                sign*basically_zero, alpha)
    ...
    def get_epsilon_delta_group(self, delta, **kwargs):
        ed_group = GraphLimitExpression.get_epsilon_delta_group(
            self, delta, **kwargs)
...
class TheoryHeavy(TeacherStudentsScene):
    def construct(self):
        ...
        Scene.play(self,
                   ...
                   *[
                       ApplyFunction(
                           lambda pi: pi.change_mode(
                               "pondering").look_at(epsilon_delta),
                           pi
                       )
