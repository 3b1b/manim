...
class DihedralGroupStructure(SymmetriesOfSquare):
    ...
    def file_away_sum(self, sum_expression):
        ...
        y = interpolate(FRAME_Y_RADIUS-LARGE_BUFF, -
                        FRAME_Y_RADIUS+LARGE_BUFF, y_prop)
...
class MultiplicativeGroupOfReals(AdditiveGroupOfReals):
    ...
    def add_number_line(self):
        ...
        self.one = [
            m for m in self.number_line.numbers if m.get_tex_string() is "1"][0]
    ...
    def follow_one(self):
        ...
        three_mob = [
            m for m in self.shadow_line.numbers if m.get_tex_string() == "3"][0]
...
class MultiplicativeGroupOfComplexNumbers(AdditiveGroupOfComplexNumbers):
    ...
    def show_action_at_i_again(self):
        neg_one_label = [
            m for m in self.real_labels if m.get_tex_string() == "-1"][0]
        half_turn_arc = Arc(
...
class ComplexExponentiationMultiplierHalf(
    ...
    def take_steps_for_e(self):
        ...
        for last_angle, angle in zip([0]+angles, angles):
            arc, brace, curved_brace, label = self.get_arc_braces_and_label(
                angle)
            if angle == np.pi:
...
class ExpTransformation(ComplexTransformationScene):
    ...
    def construct(self):
        ...
        cylinder = self.plane.copy().apply_function(
            lambda x_y_z: np.array(
                [x_y_z[0], np.sin(x_y_z[1]), -np.cos(x_y_z[1])])
        )
