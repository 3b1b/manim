...
class FirstLighthouseScene(PiCreatureScene):
    ...
    def show_lighthouses_on_number_line(self):
        ...
        randy.next_to(origin_point, LEFT, buff=0,
        ...
        for (i, light_source) in zip(list(range(NUM_VISIBLE_CONES)), light_sources[:NUM_VISIBLE_CONES]):
            indicator_start_time = 1.0 * \
                (i+1) * SWITCH_ON_RUN_TIME / \
                light_source.radius * self.number_line.unit_size
        ...
        for (i, light_source) in zip(list(range(NUM_VISIBLE_CONES, NUM_CONES)), light_sources[NUM_VISIBLE_CONES:NUM_CONES]):
            ...
            self.play(
                ...
                ApplyMethod(light_indicator.foreground.set_fill,
                            None, opacities[i-1])
            )
...
class EarthScene(Scene):
    ...
    def construct(self):
        ...
        # screen
        self.screen = VMobject(
            stroke_color=WHITE, stroke_width=SCREEN_THICKNESS)
...
class ScreenShapingScene(ThreeDScene):
    ...
    def left_shift_screen_while_showing_light_indicator(self):
        ...
        self.left_shift = (self.screen.get_center()[
                           0] - self.spotlight.get_source_point()[0])/2
    ...
    def right_shift_screen_while_showing_light_indicator_and_distance_arrow(self):
        ...
        self.play(
            ...
            ApplyMethod(self.indicator.set_intensity,
                        self.indicator_intensity),
    ...
    def morph_into_3d(self):
        ...
        distance1 = distance0
        target_point = self.camera.get_spherical_coords(
            phi1, theta1, distance1)

        dphi = phi1 - phi0
        dtheta = theta1 - theta0

        # self.camera.get_spherical_coords(45 * DEGREES, -60 * DEGREES)
        camera_target_point = target_point
        projection_direction = self.camera.spherical_coords_to_point(
            phi1, theta1, 1)
    ...
    def prove_inverse_square_law(self):
        ...
        fourfold_screen.scale(
            2, about_point=self.light_source.get_source_point())
        ...
        ninefold_screen.scale(
            3, about_point=self.light_source.get_source_point())
    ...
    def construct(self):
        ...
        for i in range(2, NUM_VISIBLE_CONES + 1):
            ...
            if bubble_indicator_target.tex_reading.get_tex_string() != "1":
                bubble_indicator_target.tex_reading.set_height(
        ...
        for i in range(NUM_VISIBLE_CONES, NUM_CONES):
            indicator_start_time = 0.5 * \
                (i+1) * FAST_SWITCH_ON_RUN_TIME / \
                light_source.ambient_light.radius * self.number_line.unit_size
            indicator_stop_time = indicator_start_time + FAST_INDICATOR_UPDATE_TIME
            indicator_rate_func = squish_rate_func(  # smooth, 0.8, 0.9)
                smooth, indicator_start_time, indicator_stop_time)
            ls = LightSource()
...
class IPTScene1(PiCreatureScene):
    ...
    def construct(self):
        ...
        screen1 = Rectangle(width=screen_width_b,
                            ...
                            stroke_width=0,
                            fill_opacity=1.0)
        screen1.move_to(C + screen_width_b/2 * RIGHT +
                        screen_thickness_b/2 * DOWN)
        ...
        screen1p.move_to(C + (screen_width_b - screen_width_bp/2)
                         * RIGHT + SCREEN_THICKNESS/2 * DOWN)
        ...
        screen2.move_to(C + screen_width_a/2 * UP +
                        screen_thickness_a/2 * LEFT)
        ...
        screen2p.move_to(C + (screen_width_a - screen_width_ap/2)
                         * UP + screen_thickness_a/2 * LEFT)
...
class IPTScene2(Scene):
    ...
    def construct(self):
        ...
        indy1 = LightIndicator(
            color=LIGHT_COLOR, show_reading=False, radius=indy_radius)
        indy1.set_intensity(intensity1)
        reading1 = TexMobject("{1\over a^2}").scale(
            formula_scale).move_to(indy1)
        indy1.add(reading1)

        indy2 = LightIndicator(
            color=LIGHT_COLOR, show_reading=False, radius=indy_radius)
        indy2.set_intensity(intensity2)
        reading2 = TexMobject("{1\over b^2}").scale(
            formula_scale).move_to(indy2)
        indy2.add(reading2)

        indy3 = LightIndicator(
            color=LIGHT_COLOR, show_reading=False, radius=indy_radius)
        indy3.set_intensity(intensity1 + intensity2)
        reading3 = TexMobject("{1\over h^2}").scale(
            formula_scale).move_to(indy3)
        indy3.add(reading3)
...
class InscribedAngleScene(ThreeDScene):
    ...
    def construct(self):
        ...
        self.ls0_dot = Dot(OBSERVER_POINT + 2 *
                           LAKE0_RADIUS * UP, fill_color=WHITE)
        ...
        original_op_func = inverse_quadratic(
            LIGHT_MAX_INT, AMBIENT_SCALE, LIGHT_CUTOFF)
        ...
        self.play(
            ...
            lake0.set_stroke, {"color": LAKE_STROKE_COLOR,
        ...
        def split_light_source(i, step, show_steps=True, run_time=1):
            ...
            if show_animation:
                ...
                self.play(
                    ApplyMethod(ls1.move_source_to,
                                ls_new_loc1, run_time=run_time),
                    ApplyMethod(ls2.move_source_to,
                                ls_new_loc2, run_time=run_time),
                )
            else:
        ...
        for i in range(4):
            ...
            point = inner_lake_center + inner_lake_radius * \
        ...
        for i in range(8):
            ...
            point = outer_lake_center + outer_lake_radius * \
                np.array([np.cos(theta), np.sin(theta), 0])
            dot = Dot(point, color=LAKE_STROKE_COLOR, radius=0.3)
            outer_ls.add(dot)
        ...
        inner_label = TexMobject("\\theta", fill_color=ANGLE_COLOR1).scale(
            3).next_to(inner_angle_arc, LEFT, buff=-0.1)
        ...
        outer_label = TexMobject("{\\theta \over 2}", color=ANGLE_COLOR2).scale(
            2.5).move_to(outer_angle_arc)
...
class PondScene(ThreeDScene):
    ...
    def construct(self):
        ...
        for n in range(3, max_it + 1):
            ...
            for i in range(2**n):
                ...
                if onscreen_old or onscreen_1:
                    ...
                    print("------------------ moving from",
                          ls_old_loc[:2], "to", pos1[:2])
                    shift_list.append(ApplyMethod(
                        ls1.move_source_to, pos1, run_time=STEP_RUN_TIME))
                else:
                    ls1.move_source_to(pos1)
                if onscreen_old or onscreen_2:
                    print("anim2 for step", n, "part", i)
                    print("------------------ moving from",
                          ls_old_loc[:2], "to", pos2[:2])
                    shift_list.append(ApplyMethod(
                        ls2.move_source_to, pos2, run_time=STEP_RUN_TIME))
                else:
        ...
        randy = Randolph(color=MAROON_D).scale(
...
class WaitScene(TeacherStudentsScene):
    ...
    def construct(self):
        ...
        self.teacher_says(TexMobject(
            "{1\over 1^2}+{1\over 3^2}+{1\over 5^2}+{1\over 7^2}+\dots = {\pi^2 \over 8}!"))

        student_q = TextMobject("What about")
        full_sum = TexMobject(
            "{1\over 1^2}+{1\over 2^2}+{1\over 3^2}+{1\over 4^2}+\dots?")
...
class ThumbnailScene(Scene):
    ...
    def construct(self):
        ...
        equation = TexMobject(
            "1+{1\over 4}+{1\over 9}+{1\over 16}+{1\over 25}+\dots")
        equation.scale(1.5)
...
class RightAnglesOverlay(Scene):
    ...
    def construct(self):
        ...
        for i in range(4):
            sign = right_angle(points[i-1], lake_center,
                               points[i], RIGHT_ANGLE_SIZE)
