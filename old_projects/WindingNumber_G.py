...
class Introduce1DFunctionCase(Scene):
    ...
    def show_axes_one_at_a_time(self):
        ...
        self.play(
            ...
            FadeOut(axes.x_axis.numbers[1], rate_func=squish_rate_func(
                smooth, 0, 0.2)),
            Write(outputs)
        )
        self.wait()
    ...
    def show_two_graphs(self):
        ...
        f_label.next_to(axes.input_to_graph_point(
            label_x_corod, f_graph), LEFT)
        ...
        for x in range(4):
            self.play(
                FadeOut(v_line.copy()),
                ShowCreation(
                    v_line, rate_func=squish_rate_func(smooth, 0.5, 1)),
                run_time=1.5
            )
        self.wait()
        self.play(
    ...
    def binary_search(
        ...
        for x in range(n_iterations - 1):
            ...
            leftovers_mid = self.compare_graphs_at_x(
                f_graph, g_graph, x_mid, **kwargs)
            ...
            if mention_signs:
                ...
                if x < n_iterations_with_sign_mention:
                    added_anims = [word.next_to,
                                   leftovers_mid[0].get_end(), -edge]
            ...
            if zoom:
                ...
                midpoint_line_index = everything.submobjects.index(
                    midpoint_line)
                everything.generate_target()
                everything.target.scale(factor, about_point=rect.get_center())
                everything.target[decimal_index].scale(
                    1./factor, about_edge=UP)
...
class TransitionFromEquationSolverToZeroFinder(Introduce1DFunctionCase):
    ...
    def transition_to_difference_graph(self):
        ...
        new_graph = axes.get_graph(
            lambda x: f_graph.underlying_function(
                x) - g_graph.underlying_function(x),
            color=GREEN
        )
...
class InputOutputScene(Scene):
    ...
    def get_planes(self):
        ...
        for plane, vect, text, color in zip(planes, vects, label_texts, label_colors):
            ...
            plane.add_coordinates(x_vals=list(
                range(-2, 3)), y_vals=list(range(-2, 3)))
...
class IntroduceInputOutputScene(InputOutputScene):
    ...
    def map_single_point_to_point(self):
        ...
        out_dot_continual_update = self.get_output_dot_continual_update(
            in_dot, out_dot)
...
class DotsHoppingToColor(InputOutputScene):
    ...
    def construct(self):
        ...
        def update_inspector_image(inspector_image):
            inspector_image.move_to(
                self.point_function(inspector.get_center()))
...
class ForeverNarrowingLoop(InputOutputScene):
    ...
    def construct(self):
        input_coloring, output_coloring = colorings = VGroup(
            *self.get_colorings())
...
class TransitionFromPathsToBoundaries(ColorMappedObjectsScene):
    ...
    def get_dot_and_add_continual_animations(self):
        ...
        def get_output(): return self.func(tuple(dot.get_center()[:2]))
        def get_output_color(): return rgba_to_color(point_to_rgba(get_output()))
        def get_output_rev(): return -point_to_rev(get_output())
        ...
        def get_total_winding(dt=0):
            ...
            i = np.argmin([abs(pw - self.curr_winding)
                           for pw in possible_windings])
...
class BreakDownLoopWithNonzeroWinding(TransitionFromPathsToBoundaries):
    def construct(self):
        ...
        arrows = VGroup(*[
            Arrow(both_cannot_be_zero.get_bottom(),
                  var.get_top(), buff=SMALL_BUFF)
            for var in (x, y)
        ])
...
class MonomialTerm(PathContainingZero):
    ...
    def show_winding(self):
        ...
        def get_in_point(): return loop.points[-1]
        def get_out_point(): return out_loop.points[-1]
        ...
        out_dot = Dot()
        update_out_dot = UpdateFromFunc(
            out_dot, lambda d: d.move_to(get_out_point()))
...
class PolynomialTerms(MonomialTerm):
    ...
    def sweep_through_loop_interior(self):
        ...
        update_out_line = UpdateFromFunc(
            out_line,
            lambda m: m.set_points(line.points).apply_function(
                self.point_function),
        )
    ...
    def cinch_loop(self):
        ...
        update_out_loop = UpdateFromFunc(
            out_loop,
            lambda m: m.set_points(loop.points).apply_function(
                self.point_function)
        )
...
class SearchSpacePerimeterVsArea(EquationSolver2d):
    ...
    def construct(self):
        ...
        def size(m): return m.get_height() + m.get_width()
...
class ShowComplexFunction(Scene):
    def construct(self):
        ...
        self.wait()
        self.play(
            ReplacementTransform(x_dot.copy(), fx_dot,
...
class WindingNumbersInInputOutputContext(PathContainingZero):
    ...
    def construct(self):
        ...
        update_out_loop = Mobject.add_updater(
            out_loop,
            lambda m: m.set_points(in_loop.points).apply_function(
                self.point_function)
        )
...
class AllOfTheVideos(Scene):
    ...
    def construct(self):
        thumbnail_dir = os.path.join(
            MEDIA_DIR, "3b1b_videos/Winding/OldThumbnails")
        n = 4
        images = Group(*[
...
class MentionQAndA(Scene):
    def construct(self):
        title = TextMobject("Q\\&A with ", "Ben", "and",
                            "Sridhar\\\\", "at", "Patreon")
...
class Thumbnail(SearchSpacePerimeterVsArea):
    ...
    def construct(self):
        ...
        lines = VGroup()
        rects = VGroup()
        def get_length(mob): return max(mob.get_width(), mob.get_height())
        for mob in mobjects:
            if mob.background_image_file is not None:
