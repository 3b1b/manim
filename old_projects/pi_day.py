...
class CircleConstants(Scene):
    ...
    def change_arc_length(self, new_angle):
        ...
        def decimal_position_update_func(decimal):
            ...
            label_radius = self.radial_label_func(
                max_radius, min_radius, angle)
            label_center = label_radius * \
        ...
        self.play(
            Rotate(self.radius, new_angle -
                   self.circle_arc.angle, about_point=ORIGIN),
...
def get_circle_drawing_terms(radius=1, positioning_func=lambda m: m.center()):
    ...
    def reposition_decimal(decimal):
        ...
        interp_length = interpolate(
            decimal.get_width(), decimal.get_height(), alpha)
    ...
    def generate_anims1(): return [ShowCreation(radius), Write(one)]

    def generate_anims2(): return [
...
class PiTauDebate(PiCreatureScene):
    def construct(self):
        ...
        terms, generate_anims1, generate_anims2 = get_circle_drawing_terms(
            radius=1.25,
            positioning_func=lambda m: m.to_edge(
                RIGHT, buff=2).to_edge(UP, buff=1)
        )
        ...
        decimal = DecimalNumber(
            np.pi, num_decimal_places=4, show_ellipsis=True)
        decimal.scale(0.75)
...
class HartlAndPalais(Scene):
    def construct(self):
        ...
        self.play(Homotopy(
            lambda x, y, z, t: (x + 0.1*np.sin(2*TAU*t) *
                                np.exp(-10*(t-0.5 - 0.5*(y-1.85))**2), y, z),
...
class ManyFormulas(Scene):
    def construct(self):
        formulas = VGroup(
            ...
            TexMobject("e^{\\tau i} = 1"),
            TexMobject(
                "n! \\approx \\sqrt{\\tau n} \\left(\\frac{n}{e} \\right)^n"),
...
class TauFalls(Scene):
    def construct(self):
        ...
        self.play(Rotate(tau, angle, rate_func=rush_into,
                         path_arc=angle, about_point=bottom))
...
class EulerWrites628(Scene):
    ...
    def construct(self):
        ...
        rect.apply_function(lambda p: np.array(
            [p[0], p[1] - 0.005*p[0]**2, p[2]]))
        ...
        perimeter.next_to(
            brace, UP, submobject_to_align=perimeter[1], buff=SMALL_BUFF)
...
class EndScene(PatreonEndScreen):
    ...
    def construct(self):
        ...
        watch_basel.next_to(basel_screen, DOWN,
