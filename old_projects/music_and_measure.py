...
class TwoChallenges(Scene):
    def construct(self):
        two_challenges = TextMobject(
            "Two Challenges", size="\\Huge").to_edge(UP)
...
class QuestionAndAnswer(Scene):
    def construct(self):
        ...
        twotwenty = TexMobject("220").scale(
            0.25).next_to(string1.mobject, LEFT)
        r220 = TexMobject("r\\times220").scale(
            0.25).next_to(string2.mobject, LEFT)
...
class PianoTuning(Scene):
    ...
    def show_interval(self, interval, half_steps):
        ...
        approximate_form = TexMobject(
            "\\approx"+str(2**(float(half_steps)/12)))
...
class PowersOfTwelfthRoot(Scene):
    def construct(self):
        ...
        for i in range(1, num_terms+1):
            ...
            error_string.scale(approx_form.get_height() /
                               error_string.get_height())
...
class AllValuesBetween1And2(NumberLineScene):
    def construct(self):
        ...
        for approx in approximations:
            ...
            self.wait(0.5)
            points = list(
                map(self.number_line.number_to_point, [approx, irrational]))
...
class StepsToSolution(IntervalScene):
    ...
    def enumerate_rationals(self):
        ...
        for frac, count in zip(rationals(), list(range(1, 28))):
            ...
            comma = TextMobject(",").next_to(
                mob_copy, buff=0.1, aligned_edge=DOWN)
...
class NotCoveredMeansCacophonous(Scene):
    def construct(self):
        ...
        statement2 = TextMobject(
            "Rationals which are close to $\\frac{\\sqrt{2}}{2}$ must have large denominators")
