...
class ProbabalisticMobjectCloud(ContinualAnimation):
    ...
    def update_mobject(self, dt):
        ...
        group = self.mobject
        points = self.gaussian_distribution_wrapper.get_random_points(
            len(group))
...
class MentionUncertaintyPrinciple(TeacherStudentsScene):
    def construct(self):
        ...
        vector_cloud = ProbabalisticVectorCloud(
            ...
            center_func=lambda: dot_cloud.gaussian_distribution_wrapper.get_parameters()[
                0],
        )
        ...
        def get_brace_text_group_update(gdw, vect, text, color):
            ...
            text = brace.get_tex(
                "2\\sigma_{\\text{%s}}" % text, buff=SMALL_BUFF)
...
class FourierTradeoff(Scene):
    ...
    def construct(self):
        ...
        fourier_graph_update = UpdateFromFunc(
            fourier_graph,
            lambda g: Transform(
                g, get_wave_packet_fourier_transform()).update(1)
        )
    ...
    def construct(self):
        ...
        self.play(
            ...
            self.get_student_changes(
                "pondering", "confused", "raise_right_hand")
        )
        self.wait(5)
...
class LongAndShortSignalsInWindingMachine(FourierRecapScene):
    ...
    def get_fourier_graph_dots(self, fourier_graph, *freqs):
        ...
        dots = VGroup()
        for freq in freqs:
            point = self.frequency_axes.input_to_graph_point(
                freq, fourier_graph)
            dot = Dot(point)
            dot.scale(0.5)
            dots.add(dot)
...
class MentionPRFNuance(TeacherStudentsScene):
    def construct(self):
        ...
        displayed_doppler_shift = TextMobject(
            "How I'm showing the \\\\", "Doppler shift")
        actual_doppler_shift = TextMobject("Actual\\\\", "Doppler shift")
        doppler_shift_words = VGroup(
    ...
    def get_spike_graph(self, x, color=RED, **kwargs):
        ...
        graph.peak_point = VectorizedPoint(
            self.axes.input_to_graph_point(x, graph))
...
class AmbiguityInLongEchos(IntroduceDopplerRadar, PiCreatureScene):
    ...
    def introduce_multiple_objects(self):
        ...
        def noise_function(t): return np.sum([
            0.5*np.sin(f*t)/f
    ...
    def fourier_transform_of_one_pulse(self):
        ...
        self.play(
            ApplyFunction(
                lambda m: m.move_to(
                    fourier_graph.saved_state).shift(-h_vect).fade(1),
...
class IntroduceDeBroglie(Scene):
    ...
    def get_wave(
        ...
        def func(x):
            dispersion = fdiv(1., self.dispersion_factor) * \
                (np.sqrt(1./(1+t0**2)))
...
class SortOfDopplerEffect(PiCreatureScene):
    ...
    def construct(self):
        ...
        def get_wave():
            ...
            def func(x): return 0.5*np.cos(omega*t - k*x)
            graph = FunctionGraph(
                func,
...
class ProbabalisticDetection(FourierTransformOfWaveFunction):
    ...
    def detect_only_single_points(self):
        ...
        question = TextMobject(
            "Do we detect the particle \\\\ in this region?")
