...
class FormalDefinitionOfContinuity(Scene):
    ...
    def setup(self):
        ...
        self.input_homotopy = lambda x_y_z_t: (
            x_y_z_t[0], x_y_z_t[1], x_y_z_t[3]) + interpolate(left, right, x_y_z_t[3])
        output_size = self.output.get_num_points()-1
        output_points = self.output.points
        self.output_homotopy = lambda x_y_z_t1: (
            x_y_z_t1[0], x_y_z_t1[1], x_y_z_t1[2]) + output_points[int(x_y_z_t1[3]*output_size)]
    ...
    def discontinuous_point(self):
        ...
        self.remove(input_circle, input_points,
                    output_circle, input_points_copy)
        self.play(vary_circles)
        self.wait()
        self.play(
        ...
        self.remove(vary_circles.mobject,
                    discontinuous_at_A, discontinuous_arrow)
...
class WonderfulPropertyOfPseudoHilbertCurves(Scene):
    def construct(self):
        ...
        for num in range(2, 9):
            ...
            new_curve.scale(0.8)
            new_dot = Dot(new_curve.points[int(
                val*new_curve.get_num_points())])
