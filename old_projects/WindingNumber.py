...
def rev_to_rgba(alpha):
    # For convenience, to go CW from red on left instead of CCW from right
    alpha = (0.5 - alpha) % 1
...
class EquationSolver1d(GraphScene, ZoomedScene):
    ...
    def drawGraph(self):
        ...
        if self.show_target_line:
            ...
            target_line_label.next_to(
                target_line_object.get_left(), UP + RIGHT)
    ...
    def solveEquation(self):
        ...
        lower_color = color_by_comparison(
            self.func(self.initial_lower_x), self.targetY)
        upper_color = color_by_comparison(
            self.func(self.initial_upper_x), self.targetY)
        ...
        # TexMobject("[") # Not using [ and ] because they end up crossing over
        startBrace = TexMobject("|", stroke_width=10)
        ...
        leftBraceLabelAnimation = ContinualChangingDecimal(leftBraceLabel,
                                                           lambda alpha: self.point_to_coords(
                                                               leftBrace.get_center())[0],
        ...
        x_guess_line = Line(lowerDotXPoint, upperDotXPoint,
        ...
        self.play(FadeIn(initialLowerXDot), FadeIn(
            leftBrace), FadeIn(leftBraceLabel))
        ...
        self.play(FadeIn(initialLowerYDot), FadeIn(
            downBrace), FadeIn(downBraceLabel))
        self.add_foreground_mobjects(initialLowerYDot, downBrace)
        self.add(downBraceLabelAnimation)

        self.wait()

        self.play(FadeIn(initialUpperXDot), FadeIn(
            rightBrace), FadeIn(rightBraceLabel))
        ...
        self.play(FadeIn(initialUpperYDot), FadeIn(
            upBrace), FadeIn(upBraceLabel))
        ...
        for i in range(self.num_iterations):
            ...
            def makeUpdater(xAtStart, fixed_guess_x):
                def updater(group, alpha):
                    ...
                    guess_line.put_start_and_end_on(
                        xAxisPoint, fixed_guess_point)
                    return group
                return updater
            ...
            midXBrace.move_to(self.coords_to_point(
                midX, self.base_line_y) + OUT)

            # We only actually add this much later
            midXPoint = Dot(self.coords_to_point(
                midX, self.base_line_y) + OUT, color=sign_color)

            x_guess_label_caption = TextMobject(
                "New guess: x = ", fill_color=midColor)
...
def plane_func_by_wind_spec(*specs):
    ...
    neg_specs_made_pos = [(x_y_z2[0], x_y_z2[1], -x_y_z2[2])
                          for x_y_z2 in neg_specs]
...
class WalkerAnimation(Animation):
    ...
    def __init__(self, walk_func, val_func, coords_to_point,
        ...
        base_walker = Dot().scale(5 * 0.35).set_stroke(walker_stroke_color,
                                                       2)  # PiCreature().scale(0.8 * 0.35)
    ...
    **kwargs
    ...
    if number_update_func != None:
        ...
        anim_group = AnimationGroup(
            walker_anim, display_anim, rate_func=linear)
        return anim_group
    else:
        return walker_anim
...
def LinearWalker(
    ...
    **kwargs
    ...
    def walk_func(alpha): return interpolate(start_coords, end_coords, alpha)
...
class ColorMappedByFuncScene(Scene):
    ...
    def setup(self):
        ...
        self.pixel_pos_to_color_func = lambda x_y3: self.pos_to_color_func(
            self.num_plane.point_to_coords_cheap(
                np.array([x_y3[0], x_y3[1], 0]))
        )
        ...
        def mini_hasher(p):
            ...
            if rgba[3] != 1.0:
                print(
                    "Warning! point_to_rgba assigns fractional alpha", rgba[3])
            return tuple(rgba)
        ...
        self.in_background_pass = not os.path.exists(
            self.background_image_file)
...
class PiWalker(ColorMappedByFuncScene):
    ...
    def construct(self):
        ...
        polygon_anim = ShowCreation(
            polygon, run_time=total_run_time, rate_func=linear)
        ...
        for i in range(len(walk_coords)):
            ...
            def val_alpha_func(a, start_coords=start_coords, end_coords=end_coords): return self.func(
                interpolate(start_coords, end_coords, a))

            if self.display_wind:
                def clockwise_val_func(p): return -point_to_rev(self.func(p))
                alpha_winder = make_alpha_winder(
                    clockwise_val_func, start_coords, end_coords, self.num_checkpoints)

                def number_update_func(alpha, alpha_winder=alpha_winder, start_wind=start_wind): return alpha_winder(
                    alpha) - alpha_winder(0) + start_wind
                start_wind = 0 if i + \
                    1 in self.wind_reset_indices else number_update_func(1)
            elif self.display_size:
                # We need to do this roundabout default argument thing to get the closure we want,
                # so the next iteration changing val_alpha_func doesn't change this closure
                def number_update_func(a, val_alpha_func=val_alpha_func): return point_to_rescaled_size(
            ...
            if self.display_odometer:
                ...
                # We need to do this roundabout default argument thing to get the closure we want,
                # so the next iteration changing val_alpha_func doesn't change this closure
                def rev_func(a, val_alpha_func=val_alpha_func): return point_to_rev(
                    val_alpha_func(a))
        ...
        if self.display_odometer:
            ...
            color_wheel.color_using_background_image(self.short_path_to_long_path(
                "pure_color_map.png"))  # Manually inserted here; this is unclean
...
class RectangleData():
    ...
    def splits_on_dim(self, dim):
        ...
        if dim == 0:
            return_data = [RectangleData(new_interval, y_interval)
                           for new_interval in split_interval(x_interval)]
        elif dim == 1:
            return_data = [RectangleData(x_interval, new_interval)
    ...
    def construct(self):
        ...
        def Animate2dSolver(cur_depth, rect, dim_to_split,
            ...
            def draw_line_return_wind(start, end, start_wind, should_linger=False, draw_line=True):
                alpha_winder = make_alpha_winder(
                    clockwise_val_func, start, end, self.num_checkpoints, cheap=self.use_cheap_winding_numbers)
                a0 = alpha_winder(0)
                def rebased_winder(alpha): return alpha_winder(
                    alpha) - a0 + start_wind
                colored_line = Line(num_plane.coords_to_point(
                    *start) + IN, num_plane.coords_to_point(*end) + IN)
                ...
                opt_line_anim = ShowCreation(
                    colored_line) if draw_line else empty_animation
            ...
            if self.show_cursor:
                ...
                cursor.move_to(num_plane.coords_to_point(
                    center_x, center_y) + 10 * IN)
            ...
            if total_wind == 0:
                ...
                points = np.array([num_plane.coords_to_point(x, y)
                                   for (x, y) in coords]) + 3 * IN
                ...
                fill_rect = polygonObject = Polygon(
                    *points, fill_opacity=0.8, color=DARK_GREY)
                ...
                mid_line_points = [num_plane.coords_to_point(
                    x, y) + 2 * IN for (x, y) in mid_line_coords]
        ...
        border = Polygon(
            *[num_plane.coords_to_point(*x) + IN for x in rect_points])
        ...
        def rect_rate(alpha):
            ...
            if time_in < 3 * run_time_base:
                ...
                alpha_in_last_leg = fdiv(
                    time_in_last_leg, run_time_with_lingering)
        ...
        print("About to do the big Play; for reference, the current time is ",
              time.strftime("%H:%M:%S"))
        ...
        print("All done; for reference, the current time is ",
              time.strftime("%H:%M:%S"))
...
class ArrowCircleTest(Scene):
    def construct(self):
        ...
        base_arrow = Arrow(circle_radius * 0.7 * RIGHT,
                           circle_radius * 1.3 * RIGHT)
        ...
        arrows = [rev_rotate(base_arrow.copy(), 0.5 - (fdiv(i, num_arrows)))
...
class OdometerScene(ColorMappedObjectsScene):
    CONFIG = {
        ...
        # This is given in terms of CW revs
        "rotate_func": lambda x: 2 * np.sin(2 * x * TAU),
        "run_time": 40,
    ...
    def construct(self):
        ...
        if self.dashed_line_angle:
            ...
            # Clockwise rotation
            dashed_line.rotate(-self.dashed_line_angle *
        ...
        def display_func(alpha): return self.rotate_func(
            alpha) + display_val_bias
    ...
    def construct(self):
        ...
        for (x, y) in points:
            ...
            normalized_output = output * \
                fdiv(self.normalized_arrow_scale_factor, output_size)
            arrow = Vector(output * self.arrow_scale_factor)
            normalized_arrow = Vector(
                normalized_output * self.arrow_scale_factor)
...
class HasItsLimitations(Scene):
    ...
    def construct(self):
        ...
        new_curved_arrow.shift(
            (new_input_dot.get_center() - new_curved_arrow.points[0]) - input_diff)
        ...
        dot_objects = Group(input_dot, input_label,
                            output_dot, output_label, curved_arrow)
        new_dot_objects = Group(new_input_dot, new_input_label,
                                new_output_dot, new_output_label, new_curved_arrow)
...
class NumberLineScene(Scene):
    def construct(self):
        ...
        rect_1d = Rectangle(stroke_width=0, fill_opacity=1,
                            fill_color=inner_color)
        rect_1d.replace(interval_1d)
        rect_1d.stretch_to_fit_height(SMALL_BUFF)
        left_dot = Dot(left_point, stroke_width=stroke_width,
                       color=border_color)
        right_dot = Dot(right_point, stroke_width=stroke_width,
                        color=border_color)
    ...
    def construct(self):
        ...
        if self.show_full_color_map:
            bright_background = Rectangle(
                width=2 * FRAME_X_RADIUS + 1, height=2 * FRAME_Y_RADIUS + 1, fill_opacity=1)
            bright_background.color_using_background_image(
                self.background_image_file)
...
class LoopSplitScene(ColorMappedObjectsScene):
    ...
    def construct(self):
        ...
        # Invisible; only used for surrounding circle
        ...
        left_square_trips = [tl_line_trip,
                             midline_left_trip, bl_line_trip, left_line_trip]
        ...
        right_square_trips = [tr_line_trip,
                              right_line_trip, br_line_trip, midline_right_trip]
        ...
        def play_fade_mid(start, end):
            play_combined_fade(
                start, end, midline_lines_vmobject, midline_bullets)
        ...
        self.wait()
        self.play(FadeOut(inner_circle), line_fade(
            1, 0, midline_lines_vmobject), *bullet_list_fade(1, 0, midline_bullets))
        self.wait()

        # Repeat for effect, goes well with narration
        self.play(FadeIn(inner_circle), line_fade(
            0, 1, midline_lines_vmobject), *bullet_list_fade(0, 1, midline_bullets))
        self.wait()
        self.play(FadeOut(inner_circle), line_fade(
            1, 0, midline_lines_vmobject), *bullet_list_fade(1, 0, midline_bullets))
        self.wait()
...
class Diagnostic(SolveX5MinusXMinus1_3Iterations):
    CONFIG = {
        ...
        # This one isn't important to set any particular way, so let's leave it like this
        "use_separate_plays": not False,
        "use_fancy_lines": True,
...
def left_func(x): return x**2 - x + 1


def diff_func(x): return np.cos(1.4 * (x - 0.1)
                                * (np.log(x + 0.1) - 0.3) * TAU)/2.1
...
tiny_loop_func = scale_func(
    plane_func_by_wind_spec((-1, -2), (1, 1), (1, 1)), 0.3)
...
class BorderOf2dRegionScene(Scene):
    def construct(self):
        ...
        interior = Polygon(*points, fill_color=neutral_color,
                           fill_opacity=1, stroke_width=0)
        self.play(FadeIn(interior))

        border = Polygon(*points, color=negative_color,
                         stroke_width=border_stroke_width)
        self.play(ShowCreation(border))


def big_loop_no_zeros_func(x_y5): return complex_to_pair(
...
class NoticeHowOnThisLoop(PiWalkerRect):
    CONFIG = {
        ...
        "start_x": 0.5,
        "start_y": -0.5,
        # We trace from bottom-right clockwise on this one, to start at a red point
        "walk_width": -1,
        "walk_height": -1,
    }
...
class UhOhScene(EquationSolver2d):
    CONFIG = {
        ...
        "manual_wind_override": (1, None, (1, None, (1, None, None))),
        "show_winding_numbers": False,
        "num_iterations": 5,
    }
...
class UhOhSalientStill(ColorMappedObjectsScene):
    ...
    def construct(self):
        ...
        midline = Line(new_up + 0.5 * new_left, 0.5 * new_left,
                       stroke_width=border_stroke_width)
        ...
        everything_filler.color_using_background_image(
            self.background_image_file)
...
class SpecifiedWinder(PiWalker):
    ...
    def setup(self):
        def rev_func(p): return point_to_rev(self.func(p))
