...
The genuinely hairy part looked like being the window: manim's interactive model is that
Python is in charge and the window is subordinate, and every GPU-canvas framework in the
ecosystem is built the other way around. The Phase 1 spike settled it more happily than
expected, see The window below: `rendercanvas`'s glfw canvas can be driven a frame at a
time through public API, so manim keeps its loop without anyone writing a window.
...
`.wgsl` and that machinery is done. WGSL has no preprocessor of its own, so `#INSERT` is not
a workaround to be deleted later; it is how WGSL is composed at any scale.

**Spiked.** Text substitution into WGSL compiles, including a `const` whose value is
substituted in, which is what decision 4 wants for `is_fill_border`. The only thing the
layout codegen has to change is what a member is called: `float` becomes `f32`, `vec3`
becomes `vec3f`, `mat4` becomes `mat4x4f`. Offsets come out identical, see decision 5.
...
**Spiked.** A read-only storage buffer is readable from the vertex stage, indexed by
`@builtin(vertex_index)` with no vertex attributes at all, which is #2482's vertex pulling
with the indirection removed.
...
`get_pipeline(shader_key, state_name, depth_test, sample_count)` behind an `lru_cache`,
mirroring today's `get_shader_program`. Note `depth_test` in the key: two of the rows above
leave depth to the mobject, so each is really two pipelines. Phase 0 shaped `DrawState` this
way already, keeping the mobject's say in it apart from the rest.

Two API details the spike turned up. A pipeline's `layout` may not be `None`; it takes a
`GPUPipelineLayout` or the string `"auto"`. And a pipeline which writes no color still needs
a fragment stage with a target, so the winding count pass asks for `write_mask: 0` rather
than leaving the fragment stage out. `VShaderWrapper.render_fill`'s forty lines of state fiddling
...
**Spiked.** WGSL's uniform layout rules agree with std140 for everything manim declares:
the 192 byte block `VMobject.uniform_dtype` already builds reads correctly as a WGSL struct,
with the last member, whose offset depends on every pad before it, landing where the dtype
says. Nothing about `uniform_block_dtype` changes.
...
- **Writing a file:** `queue.read_texture` → `np.frombuffer`. The 256 byte row alignment a
  texture copy wants is handled inside wgpu-py, spiked at low quality's awkward 854 × 4 =
  3416 byte rows, so there is nothing to pad or slice by hand.
...
backends — is documented around registering a draw callback and calling `loop.run()`, which
manim cannot do: its loop is what `self.wait` and `self.embed` are. The plan was therefore to
...
scale factors, resize, and presentation. What manim keeps is the part it cares about, being
the thing that decides when a frame happens.

`window.py` then wraps a `RenderCanvas` rather than subclassing a `moderngl_window` window,
and the workarounds go with the framework that needed them:
...
| two blits and two `swap_buffers` per frame | `context.get_current_texture()`, presented for us |

`moderngl_window`, `pyglet` and `screeninfo` all drop out; `rendercanvas` comes in as a
dependency of `wgpu` which is already installed.
...
currently ends with `result.w = 1.0 - result.z; result.z *= -0.1;`.

**Spiked.** The same ordering in `[0, w]` is the midpoint of GL's `z` and `w`, which is the
usual conversion `clip.z → (clip.z + clip.w) / 2`, and here comes to one line:

```wgsl
    out.position.w = 1.0 - z;
    out.position.z = 0.5 * (1.0 - 1.1 * z);   // = (w - 0.1 * z) / 2
```

Three squares placed at three depths and listed nearest first, so that painting them in
order would put the furthest on top, come out correctly ordered with it, and the mapping GL
asked for loses everything nearer than the origin outright rather than subtly, which is a
better failure than feared. It still wants validating against a real `ThreeDScene`, since
what the spike checked was the mapping and not the view matrix feeding it.
...
while `discard` cuts per-pixel — nearly identical, differing only in where the boundary
falls by up to a pixel.

**Landed and spiked.** Phase 0c made this change under GL, where it was A/B testable: the
difference showed up in the two clip plane cases of the comparison suite and nowhere else,
and cost nothing measurable. The spike then confirmed the part the port depends on, that a
thrown away fragment writes no stencil, by discarding half a filled star and finding the
kept half filled and the other half untouched.
...
### Phase 1 — spike, one throwaway file — **done**

`wgpu_spike.py`, eleven checks, each printing whether what this plan assumed held: the Metal
backend, vertex pulling from a storage buffer, a uniform block from the existing dtype,
stencil-then-cover, the same with four samples, discard suppressing stencil writes, separate
alpha blending, readback at an awkward row width, the depth remap and the mapping it
replaces, error reporting, and a window driven a frame at a time. All eleven hold. Delete the
file when Phase 2 lands; until then it is the evidence behind the notes above.
...
- **wgpu-py maturity.** Pre-1.0, with API renames between versions, so it is pinned above
  at what the spike ran against. Errors are better than feared: both a shader which will not
  compile and a pipeline whose bind groups do not match what its shader declares raise
  `GPUValidationError` from the call that caused them, with wgpu-native's message attached.
  Losing moderngl's friendlier wording is a real but small cost.
- ~~**MSAA with stencil-then-cover.**~~ Spiked: four samples with a
  `depth24plus-stencil8` attachment counts and covers correctly, and resolves to
  anti-aliased edges.
