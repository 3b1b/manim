...
class EsquemaManim(Scene):
	...
	def recuadro(self,texto,color):
		...
		text.add_background_rectangle(opacity=1,color=color,buff=SMALL_BUFF)
		return text
...
class Temario(Scene):
	def construct(self):
		titulo=Texto("\\sc Temario").scale(2).to_edge(UP)
		ul=underline(titulo)
		lista=VGroup(
			...
			).arrange_submobjects(DOWN,aligned_edge=LEFT).next_to(titulo,DOWN)
